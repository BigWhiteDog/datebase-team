					...
					if(temp_select_query.join_sign){//link condition
						if(ind6>0){//has and
							...
							if(strchr(sub_str,"and")){//3 conditions in total
								get_middle(condition[1], sub_str,"\0"," and");
								get_middle(condition[2], sub_str, "and ", "\0");
							}
							else{//only 2 conditions
								strcpy(condition[1], sub_str);
							}
						}
						...
						if(strchr(condition[2],'.')){//tname.cname
							...
							if(res2){ printf("Column %s doesn’t exist\n", temp_col_name[1]); continue;}
							if(err) continue;
							if (!strcmp(table_head_p[temp_select_query.use_table_no[0]].col_name[temp_select_query.join_table_0_col_no], table_head_p[temp_select_query.use_table_no[0]].col_name[temp_select_query.join_table_1_col_no])){ printf("Syntax error\n"); continue; }
							if (!strcmp(table_head_p[temp_select_query.use_table_no[1]].col_name[temp_select_query.join_table_0_col_no], table_head_p[temp_select_query.use_table_no[1]].col_name[temp_select_query.join_table_1_col_no])){ printf("Syntax error\n"); continue; }
						}
					}
...
void get_shell(char * shell, FILE * sql){
	...
	while(1){
		...
		for (i = 0; i < strlen(buf); i++){
			...
			else if ((!(buf[i] == ' '&& (((b_1>='A' && b_1<= 'Z') || (b_1 >='a' && b_1 <='z')) && ((buf[i+1]>='A' && buf[i+1]<= 'Z') || (buf[i+1] >='a' && buf[i+1] <='z')))||(b_1>='0'&& b_1<='9') )) && in_quort == 0 && i==0)// only the blank space between 'word' and 'word' will be preseved
				continue; //in this way , the colname and table name must write in words
